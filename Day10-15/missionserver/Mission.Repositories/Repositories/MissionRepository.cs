    ...
    public class MissionRepository(MissionDbContext dbContext) : IMissionRepository
    {
       ...
       .ToList();
        ...
        public List<MissionApplicationDto> GetMissionApplicationList()
        {
            ...
            //        .Where(m => m.Id == x.MissionId)
            //        .Select(m => m.MissionTitle)
            //        .FirstOrDefault(),
            //    MissionTheme = dbContext.MissionThemes
            //        .Where(mt => mt.Id == dbContext.Missions
            //            .Where(m => m.Id == x.MissionId)
            //            .Select(m => m.MissionThemeId)
            //            .FirstOrDefault())
            //        .Select(mt => mt.ThemeName)
            //        .FirstOrDefault(),
            //    FirstName = dbContext.User
        ...
        {
            dbContext.MissionApplications
                .Where(ms => ms.Id == id)
                .ExecuteUpdate(ms => ms.SetProperty(property => property.IsDeleted, true));

            return "Delete Mission Application Successfully..";
        }
        //public async Task<bool> DeleteMissionApplication(int missionAppId)
        //{
        //    var missionThemeExistingInDb = await dbContext.MissionApplications.FindAsync(missionAppId);

        //    if (missionThemeExistingInDb == null)
        //        return false;

        //    dbContext.MissionApplications
        //            .Where(ms => ms.Id == missionAppId)
        //           .ExecuteUpdate(ms => ms.SetProperty(property => property.IsDeleted, true));
        //      await dbContext.SaveChangesAsync();
        //    return true;
        //}
