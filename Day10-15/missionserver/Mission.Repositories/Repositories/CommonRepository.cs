...
namespace Mission.Repositories.Repositories
{
    public class CommonRepository(MissionDbContext cIDbContext) : ICommonRepository
    {
        ...
        {
            var userSkill = _cIDbContext.UserSkills
                .Where(m => m.UserId == userId && !string.IsNullOrEmpty(m.Skill)) 
                .Select(m => new DropDownResponseModel(m.Id, m.Skill))
                .ToList();
        ...
        public List<DropDownResponseModel> UserAvailability(int userId)
        {
            return _cIDbContext.UserDetails
                .Where(m => m.UserId == userId)
                .Select(m => new DropDownResponseModel(m.Id, m.Availability))  
                .ToList();
        }
    }
}
