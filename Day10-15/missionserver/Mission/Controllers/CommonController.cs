...
namespace Mission.Api.Controllers
{
    ...
    [ApiController]
    public class CommonController(ICommonService commonService, IWebHostEnvironment hostingEnvironment, MissionDbContext dbContext) : ControllerBase
    {
            ...
            try
            {
                if (files != null && files.Count > 0)
                {
                    foreach (var file in files)
                    {
                        ...
                        var fPath = Path.Combine("UploadedImages", "Mission");
                        string fileRootPath = Path.Combine(Directory.GetCurrentDirectory(),"UploadedImages", "Mission");
        ...
        [HttpGet]
        [Route("UserAvailability/{userId}")]
        public ResponseResult UserAvailability(int userId)
        {
            try
            {
                result.Data = _commonService.UserAvailability(userId);
                result.Result = ResponseStatus.Success;
            }
            catch (Exception ex)
            {
