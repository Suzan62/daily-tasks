...
export class UserEditProfileComponent implements OnInit, OnDestroy {
  ...
  getCountryList() {
    const countryListSubscribe = this._commonService.getMissionCountryList().subscribe((data: any) => {
      if (data.result == 1) {
  ...
  getCityList(countryId: any) {
    ...
    const cityListSubscribe = this._commonService.getMissionCityList().subscribe((data: any) => {
      if (data.result == 1) {
...
onImageError(event: any) {
  event.target.src = 'assets/NoUser.png'; // Fallback image
}
imageUrl: string = APP_CONFIG.imageBaseUrl;

getUserImageUrl(userImage: string): string {
  if (!userImage || userImage === 'default.png') {
    return 'assets/NoUser.png'; // Fallback image from Angular assets
  }
  return `${this.imageUrl}/UploadedImages/${userImage}`; // Backend-served images
}
//
  userFormCheckValid() {
...
onSubmitChangePassword(changePasswordForm: NgForm) {
  ...
  if (changePasswordForm.valid) {
    this._loginService.changePassword(value).subscribe(
      (data: any) => {
