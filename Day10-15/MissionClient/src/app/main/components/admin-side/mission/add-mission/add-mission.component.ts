  ...
  onSelectedImage(event: any) {
    ...
    if (files) {
      ...
      for (const file of files) {
        ...
        reader.onload = (e: any) => {
              // const imageName = file.name.replace(/\s/g, ""); // Ensure no spaces
                // const imageUrl = `${APP_CONFIG.imageBaseUrl}/UplodedImages/Mission/${imageName}`;
  ...
  async onSubmit() {
    ...
    if (this.addMissionForm.valid) {
      ...
      let imgUrlList = imageUrl.map(e => e.replace(/\s/g, "")).join(",");
            // let imgUrlList = imageUrl.map(imageName => `${APP_CONFIG.imageBaseUrl}/UplodedImages/Mission/${imageName.replace(/\s/g, "")}`);
      value.missionImages = imgUrlList;
        // this.addMissionForm.get('missionImages')?.setValue(imgUrlList);
      const addMissionSubscription = this._service.addMission(value).subscribe((data: any) => {

        if (data.result == 1) {
          this._toast.success({ detail: "SUCCESS",summary: typeof data.data === 'string' ? data.data : "Mission added successfully", 
